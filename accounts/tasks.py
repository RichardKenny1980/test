...
def sync_workspace_directory():
    ...
    a Gmail + Chat + Calendar sync for each. Google Tasks isn't covered - it
    has no domain-wide-delegation support, so gtasks.tasks.sync_all_tasks
    (the per-user OAuth path) still owns Tasks regardless of this task.
    """
    ...
    from gcal.tasks import sync_calendar_for_user
